...
spool C:\DATABASE\Meet_Final_Q4_spool.txt
select to_char(sysdate, 'Day DD Month Year HH:MI:SS AM') FROM dual;

--PROJECT PART 7

create or replace FORCE view old_order_detail as 
select order_line_archive.old_o_id, order_line_archive.old_inv_id, order_line_archive.old_ol_quantity, o_date, o_methpmt, item_desc , c_last, c_first
from customer, orders, order_line_archive, inventory, item
where inventory.item_id = item.item_id
and order_line_archive.old_inv_id = inv_id
and order_line_archive.old_o_id = o_id
and customer.c_id = orders.c_id;

--PROOF
create table order_line_archive(old_inv_id number, old_o_id NUMBER , old_ol_quantity number,constraint PK_OLA primary key (old_o_id, old_inv_id),
                    constraint fk1 foreign key (old_o_id) references orders(o_id),constraint fk2 foreign key (old_inv_id) references inventory(inv_id));
...
drop table order_line_archive cascade constraints; 
